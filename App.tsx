            ...
            const loadData = async () => {
                try {
                    const [txs, banks, prods, custs, servs, closings, supps, teamMembers] = await Promise.all([
                        dataService.fetch(currentUser.storeId!, 'transactions'),
                        dataService.fetch(currentUser.storeId!, 'bank-accounts'),
                        dataService.fetch(currentUser.storeId!, 'products'),
                        dataService.fetch(currentUser.storeId!, 'customers'),
                        dataService.fetch(currentUser.storeId!, 'service-orders'),
                        dataService.fetch(currentUser.storeId!, 'cash-closings'),
                        dataService.fetch(currentUser.storeId!, 'suppliers'),
                        authService.getStoreTeam(currentUser.storeId!)
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden relative">
                ...
                <main className="flex-1 overflow-hidden relative flex flex-col">
                    <Routes>
                        ...
                        />} />
                        <Route path="reports" element={<ReportsPage transactions={transactions} summary={financialSummary} storeName={settings.storeName} />} />
