    ...
    return (
        <div className="flex-1 overflow-y-auto px-4 md:px-8 py-8 bg-slate-50 dark:bg-background-dark pb-32 font-display">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
                <div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={togglePrivacy} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
                            <span className="material-symbols-outlined text-lg">{privacyMode ? 'visibility_off' : 'visibility'}</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto items-stretch md:items-center">
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            ...
                            <span className="md:hidden lg:inline">Fechar</span>
                        </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <StatCard icon="payments" color="emerald" label={viewMode === 'DAY' ? "Entradas (Dia)" : "Receita (Mês)"} value={stats.revenue} hidden={privacyMode} />
                <StatCard icon="trending_down" color="rose" label={viewMode === 'DAY' ? "Saídas (Dia)" : "Despesas (Mês)"} value={stats.expenses} hidden={privacyMode} />
                <StatCard icon="account_balance_wallet" color={stats.profit >= 0 ? "blue" : "rose"} label="Saldo Líquido" value={stats.profit} highlight={true} hidden={privacyMode} />
                
                {/* Card de Performance Combinado */}
                <div className="bg-white dark:bg-card-dark p-1 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-between">
                    <div className="flex-1 p-3 flex items-center justify-between border-b border-slate-50 dark:border-slate-800">
                        <div>
                            ...
                            <p className="text-lg font-black text-slate-900 dark:text-white">
                                {privacyMode ? 'R$ •••' : formatCurrency(extraStats.salesTotal)}
                            </p>
                        </div>
                    ...
                    <div className="flex-1 p-3 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-lg font-black text-slate-900 dark:text-white">
                                {privacyMode ? 'R$ •••' : formatCurrency(extraStats.avgTicket)}
                            </p>
                        </div>
            ...
            <div className="w-full bg-white dark:bg-card-dark p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm mb-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-white dark:bg-card-dark p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col h-[350px]">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                        <p className="text-sm font-black text-slate-900 dark:text-white">
                                            {privacyMode ? 'R$ •••' : formatCurrency(method.total)}
                                        </p>
                                    </div>
                                </div>
                                    ...
                                    <tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-right align-top">
                                            <span className={`text-sm font-black ${tx.type === 'INCOME' ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                {privacyMode ? 'R$ •••••' : (tx.type === 'INCOME' ? '+' : '-') + ' ' + formatCurrency(tx.amount)}
                                            </span>
                                        </td>
...
const StatCard = ({ icon, label, value, hidden, highlight, color }: any) => {
    ...
    const bgClass = highlight ? 'bg-primary text-white border-primary shadow-orange-500/20' : 'bg-white dark:bg-card-dark border-slate-100 dark:border-slate-800';
    ...
    return (
        <div className={`p-6 rounded-2xl border shadow-sm flex flex-col relative overflow-hidden ${bgClass}`}>
            ...
            <h3 className={`text-3xl font-black mt-4 relative z-10 ${textClass}`}>
                {hidden ? <span className="blur-sm opacity-50">R$ •••••</span> : formatCurrency(value)}
            </h3>
        </div>
    );
};
